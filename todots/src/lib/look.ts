 ...
 *
 * Тема живёт в профиле, чтобы переезжать между устройствами, но профиль
 * приходит из базы, а это секунда-две. Всё это время человек с тёмной темой
 * смотрел на белый экран ожидания, а потом получал вспышку перекраски.
 * Поэтому выбранное запоминаем ещё и на самом устройстве и применяем до
 * первой отрисовки.
 */
export const THEME_KEY = "todots.theme";

export const BACKGROUNDS = { light: "#f2f2f7", dark: "#0b0f16" };

/** Запомнить выбор на этом устройстве. Приватный режим молча игнорируем. */
export function rememberTheme(theme: string | undefined): void {
  try {
